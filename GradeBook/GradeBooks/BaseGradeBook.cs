...
namespace GradeBook.GradeBooks
{
    public abstract class BaseGradeBook
    {
        ...
        {
            var weightedAdd = IsWeighted && (studentType == StudentType.Honors || studentType == StudentType.DualEnrolled) ? 1 : 0;
            switch (letterGrade)
            {
                case 'A':
                    return 4 + weightedAdd;
                case 'B':
                    return 3 + weightedAdd;
                case 'C':
                    return 2 + weightedAdd;
                case 'D':
                    return 1 + weightedAdd;
                case 'F':
                    return 0 + weightedAdd;
            }
            return 0;
        }
